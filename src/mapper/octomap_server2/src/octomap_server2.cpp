...
#include <octomap_server2/octomap_server2.hpp>
...
namespace octomap_server {
    OctomapServer::OctomapServer(
        ...
        m_pointcloudMinHeight(0.1),
        m_pointcloudMaxHeight(1.5),
        ...
        m_pointcloudMinHeight = this->declare_parameter(
    "pointcloud_min_height", m_pointcloudMinHeight);

m_pointcloudMaxHeight = this->declare_parameter(
    "pointcloud_max_height", m_pointcloudMaxHeight);
...
rclcpp::QoS qos = rclcpp::QoS(rclcpp::KeepLast(3))
    ...
    void OctomapServer::insertCloudCallback(
        ...
        if (m_filterGroundPlane) {
            ...
            // ✅ 新增：高度过滤
        pcl::PassThrough<PCLPoint> pass_height;
        pass_height.setFilterFieldName("z");
        pass_height.setFilterLimits(m_pointcloudMinHeight, m_pointcloudMaxHeight);
        pass_height.setInputCloud(pc.makeShared());
        pass_height.filter(pc);
