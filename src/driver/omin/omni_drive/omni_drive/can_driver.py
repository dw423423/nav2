...
class CANDriver:
    ...
    def init_can_bus(self):
        """
        初始化CAN总线连接（python-can / socketcan）
        """
        try:
            ...
            # 创建 python-can 的 Bus（Linux SocketCAN）
            self.bus = can.interface.Bus(channel=self.channel, bustype='socketcan')

            self.logger.info(f"CAN接口准备就绪: {self.channel} @ {self.bitrate}bps (python-can)")
            return True
    ...
    def send_control_command(self, address, motor1_speed, motor2_speed, control_mode=0x03):
        ...
        try:
            if self.bus is None:
                self.logger.error("CAN bus 未初始化，无法发送")
                return False
            ...
            # 功能配置位: 进入运行状态 (01)
            packet2.append(0x02)          # Bit0=0(不清零), Bit2&Bit1=01(运行状态)
            packet2.append(0x00)          # 保留

            # 计算CRC (简单求和校验) - 保持你原来的算法/范围
            ...
            self.logger.info(f"  消息1 ID: {address:03X}, Data: {packet1_hex}")
            self.logger.info(f"  消息2 ID: {address:03X}, Data: {packet2_hex}")

            # 发送（标准帧 11-bit）
            msg1 = can.Message(arbitration_id=address, data=bytes(packet1), is_extended_id=False)
            msg2 = can.Message(arbitration_id=address, data=bytes(packet2), is_extended_id=False)

            self.bus.send(msg1, timeout=0.01)
            self.bus.send(msg2, timeout=0.01)
            ...
            self.logger.error(f"python-can 发送失败: {e}")
            return False
        except Exception as e:
    ...
    def close(self):
        ...
        try:
            ...
            self.logger.warning(f"关闭CAN bus 失败: {e}")
...
#             # 转换为int16
#             motor1_speed_int = int(round(motor1_speed))
#             motor2_speed_int = int(round(motor2_speed))
