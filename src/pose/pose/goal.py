...
#         navigator.get_logger().info('未提供目标坐标，使用默认值...')
#         target_x = 0.019   # 从tf信息中提取的x坐标
#         target_y = -0.002  # 从tf信息中提取的y坐标
#         target_yaw = 0.005 # 从tf信息中提取的偏航角（度）
...
class SinglePointNavigator(Node):
    def __init__(self):
        ...
        # 定义起始点和目标点
        self.start_point = {'x': 0.0, 'y': 0.0, 'yaw': 0.0}  # 起始点坐标
        self.target_point = {'x': 2.0, 'y': 0.0, 'yaw': 0.0}  # 目标点坐标，改为更远的距离
        
        # 当前目标点索引，0表示起始点，1表示目标点
        self.current_target_index = 1  # 初始目标是目标点
    ...
    def get_result_callback(self, future):
        ...
        try:
            ...
            # 添加短暂延时，确保机器人完全停止后再发送下一个目标
            time.sleep(5.0)
            
            # 导航完成后切换目标点，继续来回运动
            self.switch_target_and_continue()
                
        except Exception as e:
            self.get_logger().error(f'获取结果时出错: {e}')

    def switch_target_and_continue(self):
        """切换目标点并继续导航"""
        # 切换目标点
        if self.current_target_index == 1:  # 当前目标是目标点，切换到起始点
            self.current_target_index = 0
            target = self.start_point
            self.get_logger().info(f'切换到起始点: ({target["x"]}, {target["y"]})')
        else:  # 当前目标是起始点，切换到目标点
            self.current_target_index = 1
            target = self.target_point
            self.get_logger().info(f'切换到目标点: ({target["x"]}, {target["y"]})')
        
        # 发送新的导航目标
        self.send_goal(target['x'], target['y'], target['yaw'])
...
def main(args=None):
    ...
    if len(sys.argv) >= 7:
        try:
            # 命令行参数: 起始点(x,y,yaw) + 目标点(x,y,yaw)
            navigator.start_point = {
                'x': float(sys.argv[1]),
                'y': float(sys.argv[2]),
                'yaw': float(sys.argv[3])
            }
            navigator.target_point = {
                'x': float(sys.argv[4]),
                'y': float(sys.argv[5]),
                'yaw': float(sys.argv[6])
            }
            navigator.get_logger().info(f'使用命令行参数设置起始点: {navigator.start_point}')
            navigator.get_logger().info(f'使用命令行参数设置目标点: {navigator.target_point}')
        except ValueError:
            navigator.get_logger().error('参数格式错误！请使用数字。')
            navigator.get_logger().info('示例: ros2 run pose goal 0.0 0.0 0.0 2.0 1.0 90.0')
            rclpy.shutdown()
            return
    else:
        navigator.get_logger().info(f'使用默认起始点: {navigator.start_point}')
        navigator.get_logger().info(f'使用默认目标点: {navigator.target_point}')
    
    # 开始来回导航，从目标点开始
    target = navigator.target_point
    navigator.send_goal(target['x'], target['y'], target['yaw'])
    
    # 保持节点运行
    try:
        rclpy.spin(navigator)
    except KeyboardInterrupt:
        navigator.get_logger().info('节点被中断')
    finally:
        navigator.destroy_node()
        rclpy.shutdown()
